...
def to_score(similarity: float, calibration: tuple[float, float] | None) -> float:
    ...
    if calibration is None:
        # Бібліотека ще замала для калібрування: показуємо сирий косинус,
        # стиснутий у шкалу, і не вдаємо точності, якої немає.
        return float(max(0.0, min(100.0, (similarity + 1.0) * 50.0)))
