...
отримуючи нічого натомість — це незмінні файли, які просто мають лежати поруч.
  ...
  python scripts/build.py --install --to "C:/Users/.../media-library"
...
def install(target_root: Path) -> None:
    """Оновлює встановлений застосунок зі свіжозібраного dist.

    Копіюється лише те, що змінилося, а тека data лишається недоторканою: у
    ній бібліотека, оригінали й налаштування. Зайві файли попередньої версії
    прибираються — лишений від старої збірки модуль імпортується мовчки.
    """
    if not (DIST / "media-library.exe").exists():
        raise SystemExit("Спершу зберіть застосунок: python scripts/build.py")

    copied = mirror(DIST, target_root, skip=("data",))

    kept = {path.relative_to(DIST) for path in DIST.rglob("*") if path.is_file()}
    stale = 0
    ...
    print(f"   оновлено файлів: {copied}" + (f", прибрано {stale}" if stale else ""))
    print(f"   бібліотека не чіпалася: {target_root / 'data'}")
...
# Межа, за якою файли звіряються за розміром, а не за вмістом. Ваги — це
# гігабайти, читати їх цілком заради порівняння дорожче, ніж перекопіювати.
# А ось код однакового розміру, але іншого змісту — цілком буденна річ, і
# пропустити його означало б розкотити оновлення, якого немає.
BY_CONTENT_BELOW = 4 * 1024 * 1024
...
def _same(left: Path, right: Path) -> bool:
    ...
    if left.stat().st_size != right.stat().st_size:
        return False
    if left.stat().st_size >= BY_CONTENT_BELOW:
        return True
...
def main() -> None:
    if "--install" in sys.argv:
        where = sys.argv[sys.argv.index("--to") + 1] if "--to" in sys.argv else None
        if not where:
            raise SystemExit("Вкажіть, що оновлювати: --install --to <тека>")
        print("Оновлюю встановлений застосунок…")
        install(Path(where))
        return
